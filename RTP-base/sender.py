...
def sender(receiver_ip: str, receiver_port: int, window_size: int):
    ...
    # seq_num -> packet
    window = {}  
    # Lưu thời gian nhận ACK cuối cùng
    ...
    while base <= chunks_length:
        ...
        # a 500 milliseconds retransmission timer to automatically retransmit packets that were never acknowledged
        timeout = 0.5 - (time.time() - last_ack_time)
        if timeout <= 0:
            for seq in window:
                s.sendto(window[seq], (receiver_ip, receiver_port))
            # reset last ack time để kiểm tra tiếp xem ACK packet có bị loss hay không 
            last_ack_time = time.time()
        else:
            readable, writable, exceptional = select.select([s], [], [], timeout)
            if readable:
                pkt, address = s.recvfrom(2048)
                header = PacketHeader(pkt[:16])
                if header.type == ACK and header.seq_num > base:
                    for seq in range(base, header.seq_num):
                        window.pop(seq, None)
                    # Cập nhật đến ACK đã nhận được gần nhất (cao nhất)
                    base = header.seq_num
                    last_ack_time = time.time()
            else:
                for seq in window:
