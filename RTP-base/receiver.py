                ...
                if header.type == DATA:
                    ...
                    if received_checksum == computed_checksum:
                        ...
                        if seq_num == expected_seq_num:
                            ...
                            print(f"Packet {seq_num} received in order, sending ACK {expected_seq_num}")
                            sys.stdout.flush()
                            
                            # send ACK with seq_num=M+1
                            ack_pkt = create_packet(ACK, expected_seq_num)
                            s.sendto(ack_pkt, address)
                        ...
                        # If it expects a packet of sequence number N and If it receives a packet with seq_num not equal to N
                            ...
                            print(f"Out-of-order packet {seq_num} received, expecting {expected_seq_num}. Buffering and sending ACK {expected_seq_num}")
                            sys.stdout.flush()
                            
                            # sends back an ACK with seq_num=N
                            ack_pkt = create_packet(ACK, expected_seq_num)
                            s.sendto(ack_pkt, address)
