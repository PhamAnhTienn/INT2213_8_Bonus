...
if [ -z "$PYTHON_PATH" ]; then
  echo "Error: Python not found. Ensure Conda is activated."
  exit 1
fi
...
PREV_PIDS=$(lsof -ti:$PORT_RECV,$PORT_SEND 2>/dev/null)
if [ -n "$PREV_PIDS" ]; then
  kill -9 $PREV_PIDS
fi
...
bash "$DIR/compare.sh" "$FOLDER_PATH/output.txt" "$DIR/test_message.txt"
